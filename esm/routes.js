import _viewsError from "./views/error";
import _viewsLogin from "./views/login";
...
const getLoginHtml = _viewsLogin;
const getErrorHtml = _viewsError;
...
function setupOauthRoutes(app, sessionStore, config = {}) {
  ...
  app.get('/oauth/callback', async (req, res, next) => {
    try {
      const params = req.method === 'POST' ? req.body : new URLSearchParams(req.url.split('?')[1]);
      const {
        session
      } = await getClient().callback(params);
      const payload = {
        sub: session.did,
      ...
      const token = jwt.sign(payload, process.env.OAUTH_JWT_SECRET, {
        expiresIn: '48h'
      });
  ...
  if (config.serveErrorPage !== false) {
    ...
    app.use(errorRoutes, (err, req, res, next) => {
      ...
      const errorMessage = process.env.NODE_ENV === 'production' ? 'An error occurred during authentication.' : err.message || 'Internal Server Error';
