...
function getConfig(config = {}) {
    ...
    const client_id = baseUrl.includes('127.0.0.1') 
        ? `http://localhost?redirect_uri=${encodeURIComponent(`${baseUrl}/oauth/callback`)}&scope=${encodeURIComponent('atproto transition:generic')}` 
...
async function initializeOAuth(config = {}, stores = {}) {
    ...
    client = new NodeOAuthClient({
        ...
        keyset: await Promise.all([
            JoseKey.fromImportable(process.env.OAUTH_PRIVATE_KEY_1, 'key1'),
            JoseKey.fromImportable(process.env.OAUTH_PRIVATE_KEY_2, 'key2'),
            JoseKey.fromImportable(process.env.OAUTH_PRIVATE_KEY_3, 'key3'),
        ]),
        stateStore,
        sessionStore,
    });
